...
public extension Color {
    ...
    static let mrtSkeletonRowBorder = Color(hex: Hex.text, alpha: 0.07)

    // MARK: - Live Activity (MYR-398 r16 redesign v2)
    //
    // The lock-screen card and the Dynamic Island are drawn by ANOTHER PROCESS
    // (`MyRoboTaxiWidgets`), which is outside this package — so every value the
    // redesign uses has to be a named token here or it becomes a hex in a view.
    // That is why this block exists at all: the design's own colour table
    // (`design/Handoff-Live-Activity.md` §4) is short, but the card ALSO spends a
    // white-opacity ladder and two gold alphas that would otherwise be typed into
    // the widget by hand.
    //
    // THE LADDER IS 100 / 62 / 42% WHITE AND NOTHING BETWEEN IT (handoff §3). It is
    // named here rather than written as `.opacity(0.62)` at each call site because
    // "nothing between" is a rule, and a rule with no name is a rule the next
    // person interpolates through.

    /// Card ground, top stop — `#1b1407`. The logo tile's own gradient, reused
    /// verbatim so the card reads as the brand tile enlarged (handoff §3).
    static let mrtActivityGroundTop = Color(hex: Hex.logoTileTop)
    /// Card ground, 55% stop — `#0d0b06`. Also the colour of the 20pt disc the
    /// rail's east arrow rides in.
    static let mrtActivityGroundMid = Color(hex: Hex.logoTileMid)
    /// Card ground, bottom stop — `#090806`.
    static let mrtActivityGroundBottom = Color(hex: Hex.logoTileBottom)
    /// The gold bloom over the ground's top-left — rgba(201,168,76,0.13), the
    /// radial stop in `radial-gradient(120% 130% at 14% -20%, …)`.
    static let mrtActivityGroundBloom = Color(hex: Hex.gold, alpha: 0.13)
    /// The card's 0.5pt gold hairline — rgba(201,168,76,0.16).
    static let mrtActivityCardHairline = Color(hex: Hex.gold, alpha: 0.16)

    /// Rail track — rgba(255,255,255,0.13).
    static let mrtActivityRailTrack = Color(hex: Hex.text, alpha: 0.13)
